...
function fetchData() {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${searchBox.value}&appid=${apiData.key}`
  ) //set data
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
...
function frontEnd__display(data) {
  ...
  if (`${data.main.temp_min - 273.15}` < 10) {
    Fspan.innerHTML =
      "0" + Math.floor(`${data.main.temp_min - 273.15}`) + "  ℃";
